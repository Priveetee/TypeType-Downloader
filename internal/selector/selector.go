...
func selectVideo(streams []typetype.VideoStreamItem, options Options) (typetype.VideoStreamItem, bool) {
	...
	for _, stream := range streams {
		...
		if !playableVideo(stream) || !strings.Contains(stream.MimeType, options.Container) || !codecMatches(codec, options.VideoCodecPrefix) {
			continue
		}
...
func selectAudio(streams []typetype.AudioStreamItem, preferredTrackID *string, options Options) (typetype.AudioStreamItem, bool) {
	...
	for _, stream := range streams {
		...
		if !playableAudio(stream) || !strings.Contains(stream.MimeType, options.Container) || !codecMatches(codec, options.AudioCodecPrefix) {
			continue
		}
